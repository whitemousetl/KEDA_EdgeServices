...
public class ModbusRtuOverTcpAdapter : ModbusProtocolAdapterBase<ModbusRtuOverTcp>
{
    protected override string ProtocolType => "ModbusRtuOverTcp";
    public ModbusRtuOverTcpAdapter(ILogger<ModbusRtuOverTcpAdapter> logger, Global global) : base(logger, global)
    {
    }
        ...
        if (_connection == null || _lastConfig == null || !_lastConfig.Equals(config) || protocol.ResetConnection)
        {
            ...
            if (!res.IsSuccess)
            {
                if (protocol.IsLogPoints)
                    _logger.LogError($"ModbusRtuOverTcp连接失败: {res.Message}");
                throw new IOException($"ModbusRtuOverTcp连接失败: {res.Message}");
            }
            _lastConfig = config;
        }
...
//public class ModbusRtuOverTcpAdapter : IProtocolAdapter
//{
//    private ModbusRtuOverTcp _connection = null!;
//    private LanConfig _lastConfig = null!;
//    private ILogger<ModbusRtuOverTcpAdapter> _logger;
//    private Global _global;
//    public ModbusRtuOverTcpAdapter(ILogger<ModbusRtuOverTcpAdapter> logger, Global global)
//    {
//        _logger = logger;
//        _global = global;
//    }
...
//        {
//            if (protocol.IsLogPoints)
//                _logger.LogError($"初始化或重置ModbusRtuOverTcp连接失败，请检查。");
//            return [];
//        }
...
//        if (_connection == null || _lastConfig == null || !_lastConfig.Equals(config) || protocol.ResetConnection)
//        {
//            _connection = new ModbusRtuOverTcp(config.Ip, config.Port)
//            {
//                ConnectTimeOut = config.ConnectTimeOut,
//                ReceiveTimeOut = config.ReceiveTimeOut,
//                AddressStartWithZero = bool.Parse(protocol.AddressStartWithZero),
...
//                    if (isLogPoints)
//                        logger.Information(messageTemplate: $"ModbusRtuOverTcp执行读取成功,参数名{point.Label},读取值{boolRes.Content},信息{boolRes.Message}");
//                }
//                else
//                {
//                    if (isLogPoints)
//                        logger.Information(messageTemplate: $"ModbusRtuOverTcp执行读取失败,参数名{point.Label},读取值{boolRes.Content},信息{boolRes.Message}");
...
//                    if (isLogPoints)
//                        logger.Information(messageTemplate: $"ModbusRtuOverTcp执行读取成功,参数名{point.Label},读取值{strRes.Content},信息{strRes.Message}");
//                }
//                else
//                {
//                    if (isLogPoints)
//                        logger.Error(messageTemplate: $"ModbusRtuOverTcp执行读取失败,参数名{point.Label},读取值{strRes.Content},信息{strRes.Message}");
//                    return false;
//                }
...
//                if (isLogPoints)
//                    logger.Error(messageTemplate: $"ModbusRtuOverTcp执行读取失败,参数名{point.Label},无法解析的数据类型");
//                return false;
//            }
//        }
//        catch (Exception ex)
...
//            readPointResult.ReadIsSuccess = false;
//            if (isLogPoints)
//                logger.Error(messageTemplate: $"ModbusRtuOverTcp执行读取失败,参数名{point.Label},异常信息{ex}");
//            return false;
//        }
...
//            }
//            if (isLogPoints)
//                logger.Information(messageTemplate: $"ModbusRtuOverTcp执行读取成功,参数名{point.Label},读取值{result.Content},实际值{readPointResult.ReadValue}，信息{result.Message}");
//            return true;
//        }
//        else
//        {
//            if (isLogPoints)
//                logger.Error(messageTemplate: $"ModbusRtuOverTcp执行读取失败,参数名{point.Label},信息{result.Message}");
//            return false;
//        }
//    }
...
//        {
//            logger.Error("ModbusRtuOverTcp的站号无法转换成byte类类型，请检查");
//            return null;
//        }
...
//            {
//                logger.Error($"ModbusRtuOverTcp的写入值为空，请检查, 参数名{point.Label}");
//                continue; // 跳过读点
//            }

//            if (!Enum.TryParse(point.DataType, out DataType dataType))
//            {
//                logger.Error($"ModbusRtuOverTcp的数据类型异常，请假查,参数名{point.Label},写入值{point.WriteValue}");
//                continue;
//            }
...
//            catch (Exception ex)
//            {
//                logger.Error($"ModbusRtuOverTcp执行写操作发生异常,参数名{point.Label},写入值{point.WriteValue},异常信息{ex}");
//            }
//        }
...
//                            logger.Information($"ModbusRtuOverTcp执行写入成功,参数名{point.Label},写入值{point.WriteValue},实际值{readRes.Content}");
//                        }
//                    }
//                    break;
//                case DataType.Short:
...
//                    break;
//                default:
//                    logger.Error($"ModbusRtuOverTcp执行写入失败，数据类型不存在,参数名{point.Label},写入值{point.WriteValue}");
//                    break;
//            }
//        }
//        catch (Exception ex)
//        {
//            logger.Error($"ModbusRtuOverTcp执行写入异常,参数名{point.Label},写入值{point.WriteValue},异常信息{ex}");
//            throw;
//        }
//    }
//}
