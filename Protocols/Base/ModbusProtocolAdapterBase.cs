...
namespace KEDA_EdgeServices.Protocols.Base;

public abstract class ModbusProtocolAdapterBase<TConnection> : IProtocolAdapter where TConnection : IReadWriteNet, IModbus
{
    protected TConnection _connection = default!;
    protected Global _global;
    protected ILogger _logger;
    protected abstract string ProtocolType { get; }
    public ModbusProtocolAdapterBase(ILogger logger, Global global)
    {
        _logger = logger;
        _global = global;
    }
    public async Task<List<DeviceDataResult>> ReadOrWriteAsync(Protocol protocol, CancellationToken ct)
    {
        ...
        if (_connection == null)
        {
            if (protocol.IsLogPoints)
                _logger.LogError($"初始化或重置{ProtocolType}连接失败，请检查。");
            return [];
        }
    ...
    protected abstract void InitOrReset(Protocol protocol);
    ...
    private bool EvaluatePointData(DeviceDataResult deviceResult, Point point, Serilog.ILogger logger, bool isLogPoints)
    {
        ...
        try
        {
            ...
            if (res is OperateResult<bool> boolRes)
            {
                ...
                if (boolRes.IsSuccess)
                {
                    ...
                    if (isLogPoints)
                        logger.Information(messageTemplate: $"{ProtocolType}执行读取成功,参数名{point.Label},读取值{boolRes.Content},信息{boolRes.Message}");
                }
                else
                {
                    if (isLogPoints)
                        logger.Information(messageTemplate: $"{ProtocolType}执行读取失败,参数名{point.Label},读取值{boolRes.Content},信息{boolRes.Message}");
            ...
            else if (res is OperateResult<string> strRes)
            {
                ...
                if (strRes.IsSuccess)
                {
                    ...
                    if (isLogPoints)
                        logger.Information(messageTemplate: $"{ProtocolType}执行读取成功,参数名{point.Label},读取值{strRes.Content},信息{strRes.Message}");
                }
                else
                {
                    if (isLogPoints)
                        logger.Error(messageTemplate: $"{ProtocolType}执行读取失败,参数名{point.Label},读取值{strRes.Content},信息{strRes.Message}");
                    return false;
                }
            ...
            else
            {
                ...
                if (isLogPoints)
                    logger.Error(messageTemplate: $"{ProtocolType}执行读取失败,参数名{point.Label},无法解析的数据类型");
                return false;
            }
        }
        catch (Exception ex)
        {
            readPointResult.ReadIsSuccess = false;
            if (isLogPoints)
                logger.Error(messageTemplate: $"{ProtocolType}执行读取失败,参数名{point.Label},异常信息{ex}");
            return false;
        }
        ...
        where T : struct, IConvertible
    {
        ...
        if (result.IsSuccess)
        {
            ...
            if (isLogPoints)
                logger.Information(messageTemplate: $"{ProtocolType}执行读取成功,参数名{point.Label},读取值{result.Content},实际值{readPointResult.ReadValue}，信息{result.Message}");
            return true;
        }
        else
        {
            if (isLogPoints)
                logger.Error(messageTemplate: $"{ProtocolType}执行读取失败,参数名{point.Label},信息{result.Message}");
            return false;
        }
    }
    ...
    private async Task<DeviceDataResult?> WriteDevicePoints(Device device, string remark)
    {
        ...
        {
            logger.Error("{ProtocoType}的站号无法转换成byte类类型，请检查");
            return null;
        }
        ...
        foreach (var point in device.Points)
        {
            ...
            {
                logger.Error($"{ProtocolType}的写入值为空，请检查, 参数名{point.Label}");
                continue; // 跳过读点
            }

            if (!Enum.TryParse(point.DataType, out DataType dataType))
            {
                logger.Error($"{ProtocolType}的数据类型异常，请假查,参数名{point.Label},写入值{point.WriteValue}");
                continue;
            }
            ...
            {
                logger.Error($"{ProtocolType}执行写操作发生异常,参数名{point.Label},写入值{point.WriteValue},异常信息{ex}");
            }
        }
        ...
        try
        {
            switch (dataType)
            {
                case DataType.Bool:
                    ...
                    //        logger.Information($"{ProtocolType}执行写入成功,参数名{point.Label},写入值{point.WriteValue},实际值{readRes.Content}");
                    //    }
                    //}
                    //if (byte.TryParse(point.WriteValue, out byte writeByte))
                    //{
                    //    var res = _connection.Write(point.Address, writeByte);
                    //    writePointResult.WriteIsSuccess = res.IsSuccess;
                    //    if (res.IsSuccess)
                    //    {
                    //        var readRes = _connection.ReadBool(point.Address);
                    ...
                    //}
                    {
                        // 支持 "true"/"false"（不区分大小写）
                        if (bool.TryParse(point.WriteValue, out bool writeBool))
                        {
                            // 已经是标准布尔字符串
                        }
                        // 支持 "1"/"0" 及数字字符串
                        else if (point.WriteValue == "1" || point.WriteValue == "1.0")
                        {
                            writeBool = true;
                        }
                        else if(point.WriteValue == "0" || point.WriteValue == "0.0")
                        {
                            writeBool = false;
                        }
                        else
                        {
                            logger.Error($"{ProtocolType}的布尔写入值无法解析为bool/int, 参数名{point.Label}, 写入值{point.WriteValue}");
                            break;
                        }
                ...
                case DataType.String:
                    ...
                    break;
                default:
                    logger.Error($"{ProtocolType}执行写入失败，数据类型不存在,参数名{point.Label},写入值{point.WriteValue}");
                    break;
            }
        }
        catch (Exception ex)
        {
            logger.Error($"{ProtocolType}执行写入异常,参数名{point.Label},写入值{point.WriteValue},异常信息{ex}");
            throw;
        }
    }
}
